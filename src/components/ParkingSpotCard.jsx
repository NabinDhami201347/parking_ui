...
const ParkingSpotCard = ({ spot }) => {
  return (
    <Link to={`/spots/${spot._id}`}>
      <div className="bg-zinc-800 shadow-lg rounded-lg overflow-hidden pb-4">
        ...
        {spot.available ? (
          <p className="px-4 py-1 bg-green-600 text-white rounded-sm">Avaliable</p>
        ) : (
          <p className="px-4 py-1 bg-red-600 text-white rounded-sm">Unavaliable</p>
        )}

        <div className="flex items-center justify-between p-2">
          <div className="">
            <h2 className="text-xl font-semibold text-gray-200">{spot.name}</h2>
            <p className="text-sm text-gray-400">{spot.location}</p>
          </div>

          <div className="">
            <p className="">{spot.spotType}</p>
            <p className=""> ${spot.pricePerHour} / hr</p>
          </div>
        </div>
